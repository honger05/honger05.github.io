...
Widget 提供了 _onRender + 属性名(首字母大写)的特性。
...
Widget 组件具有四个要素：描述状态的 attributes 和 properties，描述行为的 events 和 methods。
...
render: function() {
  ...
  if (parentNode && !isInDocument(this.element[0])) {
    ...
    var oterBoxClass = this.constructor.outerBoxClass;
    if (outerBoxClass) {
...
-onRenderClassName: function(val) {
